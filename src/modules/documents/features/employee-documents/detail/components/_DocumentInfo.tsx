...
import type { DocumentDetailData } from '../actions.server';
import { getHistoryVersionDownloadUrl } from '../actions.server';
...
export function _DocumentInfo({ data }: Props) {
  const { document, employee, appliesCurrently } = data;
  const badgeConfig = getDocumentStateBadge(document.state, document.documentType.hasExpiration);
  const [loadingHistoryId, setLoadingHistoryId] = useState<string | null>(null);

  const handleDownloadHistoryVersion = async (historyId: string, fileName: string | null) => {
    setLoadingHistoryId(historyId);
    try {
      const result = await getHistoryVersionDownloadUrl(historyId, document.id, employee.id);
  ...
  return (
    <div className="space-y-6">
      {/* Alerta si el documento ya no aplica al empleado */}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <p className="font-medium text-yellow-800 dark:text-yellow-200">
                  Este documento ya no aplica al empleado
                </p>
                <p className="text-sm text-yellow-700 dark:text-yellow-300">
                  Las condiciones del tipo de documento han cambiado o el empleado ya no cumple con
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <div>
              ...
              <CardDescription>
                {document.period ? `Periodo: ${document.period}` : 'Documento del empleado'}
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3 rounded-lg border p-3">
            <Avatar className="h-10 w-10">
              <AvatarImage src={employee.pictureUrl || undefined} />
              <AvatarFallback>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground truncate">
                Legajo: {employee.employeeNumber} · DNI: {employee.documentNumber}
              </p>
            </div>
              ...
              <div className="relative">
                ...
                <div className="space-y-4">
                  {document.history.map((entry, index) => {
                    ...
                    // Las entradas RENEWED guardan el archivo anterior (se puede descargar)
                    // Las entradas REPLACED/UPLOADED del pasado también si tienen documentKey diferente al actual
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleDownloadHistoryVersion(entry.id, entry.fileName)
                                    }
