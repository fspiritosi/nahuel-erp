...
import { _EquipmentDocumentUploadFormWithMode } from './_EquipmentDocumentUploadFormWithMode';
import { getEquipmentDocumentDownloadUrl } from '../../list/actions.server';
...
export function _EquipmentDocumentActions({
  documentId,
  equipmentId,
  documentTypeId,
  documentTypeName,
  ...
  const revertMutation = useMutation({
    mutationFn: () => revertEquipmentDocumentVersion(documentId, equipmentId),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({ queryKey: ['equipmentDocuments', equipmentId] });
        queryClient.invalidateQueries({ queryKey: ['documentDetail', documentId, equipmentId] });
  ...
  const handleDownload = async () => {
    try {
      const result = await getEquipmentDocumentDownloadUrl(documentId, equipmentId);
  ...
  return (
    <>
      ...
      {/* Upload Dialog */}
      <_EquipmentDocumentUploadFormWithMode
